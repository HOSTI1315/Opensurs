...
local esp = {
    ...
    --\ @enemies
    enemy_chams = { false, Color3.new(1, 1, 1), Color3.new(0,0,0), .25, .75, true },
    --
    ...
    textsize = 13,
    --
    snapline = {false, Color3.new(1,1,1), nil},
...
function esp:CFrameToViewport(cframe)
    return camera:WorldToViewportPoint(cframe * (cframe - cframe.p):ToObjectSpace(camera.CFrame - camera.CFrame.Position).Position)
end
...
function esp:add(plr)
    ...
    local objs = {
        ...
        --
        snapline = esp:draw("Line", {Thickness = 1})
    }
...
function esp:update()
    for plr, drawing in next, esp.players do
        ...
        if esp.enabled and esp.checkalive(player) then
            ...
            -- snapline
            drawing.snapline.Visible = esp.snapline[1] and esp.snapline[3] ~= nil
            if drawing.snapline.Visible then
                local to, toVis = esp:CFrameToViewport(esp.snapline[3].CFrame)
                drawing.snapline.Visible = toVis
                drawing.snapline.From = camera.ViewportSize / 2
                drawing.snapline.To = NEWVEC2(to.X, to.Y)
                drawing.snapline.Color = esp.snapline[2]
            end
