...
local esp = {
    ...
    --\ @enemies
    chams = { false, Color3.new(0, 0, 0), Color3.new(1, 1, 1), 0, 0, true },
    --
    boxes = { false, Color3.new(1,1,1), Color3.new(), 0.15 },
    healthbar = { false, Color3.new(), Color3.new() },
    arrow = { false, Color3.new(), 0.5 },
    --
    names = { false, Color3.new()},
    weapon = { false, Color3.new()},
    distance = false,
    health = false,
...
function esp:fadeviadistance(data)
    return data.limit and 1 - CLAMP(self:convertnumrange(FLOOR(((data.cframe.p - camera.CFrame.p)).magnitude), (data.maxdistance - data.factor), data.maxdistance, 0, 1), 0, 1) or 0;
end;
...
function esp:add(plr)
    ...
    local objs = {
        ...
        box = esp:draw('Square', { Filled = false, Thickness = 0, Color = NEWCOLOR3(1,1,1) }),
...
function esp:update()
    for plr, drawing in next, esp.players do
        ...
        if esp.enabled and esp.checkalive(player) then
            ...
            -- arrows
            drawing.arrow.Visible = esp.arrow[1] and pass and not onScreen
            if drawing.arrow.Visible then
                ...
                drawing.arrow.Color = esp.arrow[2];
                drawing.arrow.Transparency = not onScreen and esp.arrow[3] or 0;
                if esp.arrowinfo then
                    local smallestX, smallestY, biggestX, biggestY = esp:returntriangleoffsets(drawing.arrow)
                    -- healthbar
                    drawing.arrow_bar.Visible = not onScreen and drawing.arrow.Visible and esp.healthbar[1]
                    drawing.arrow_bar_inline.Visible = drawing.arrow_bar.Visible
                    drawing.arrow_bar_outline.Visible = true
                    if drawing.arrow_bar.Visible then
                        drawing.arrow_bar.Color = esp.healthbar[3]:Lerp(esp.healthbar[2], health / 100)
            ...
            -- chams
            drawing.chams.ins.Enabled = esp['chams'][1] and pass
            drawing.chams.ins.Adornee = esp['chams'][1] and player.Character or nil
            drawing.chams.ins.Parent = folder
            if drawing.chams.ins.Enabled then
                drawing.chams.ins.FillColor = esp['chams'][2]
                drawing.chams.ins.OutlineColor = esp['chams'][3]
                drawing.chams.ins.FillTransparency = esp['chams'][4]
                drawing.chams.ins.OutlineTransparency = esp['chams'][5]
                drawing.chams.ins.DepthMode = esp['chams'][6] and Enum.HighlightDepthMode.AlwaysOnTop or Enum.HighlightDepthMode.Occluded
            end;
            ...
            if drawing.bar.Visible then
                drawing.bar.Color = esp['healthbar'][3]:Lerp(esp['healthbar'][2], health / 100)
            ...
            -- distance
            drawing.distance.Visible = not esp['names'][1] and esp['distance']
            ...
            if drawing.distance.Visible then
                ...
                drawing.distance.Color = esp['names'][2]
            ...
            if drawing.name.Visible then
                drawing.name.Text = esp['distance'] and '['..distance..'] '..playerName or playerName
                drawing.name.Font = Drawing.Fonts[esp.font]
                drawing.name.Size = esp.textsize
                drawing.name.Color = esp['names'][2]
            ...
            if drawing.health.Visible then
                ...
                drawing.health.Color = esp['healthbar'][3]:Lerp(esp['healthbar'][2], health / 100)
            ...
            -- weapon
            drawing.weapon.Visible = esp['weapon'][1]
            ...
            if drawing.weapon.Visible then
                ...
                drawing.weapon.Color = esp['weapon'][2]
